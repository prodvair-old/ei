...
class EtpController extends Controller
{
    ...
    public function actionList($q = null, $id = null)
    {
        ...
        if (is_null($id)) {
            $etps = Etp::find()->limit(6);

            if (!is_null($q)) {
                $etps->where(['like', 'lower("title")', mb_strtolower($q, 'UTF-8')]);
            }

            foreach ($etps->all() as $etp) {
                $etpsList['results'][] = [
                    'id' => $etp->id,
                    'text' => '<b>'.$etp->title.'</b>'
                ];
            ...
            $etp = Owners::findOne(['id' => $id]);

            $etpsList['results'] = ['id' => $id, 'text' => '<b>'.$etp->title.'</b>'];
        }

  ...
  <ul>
    ...
    <? if (Yii::$app->user->identity->role == 'agent') { ?>
      ...
      <ul <?= (strpos($page, 'lot') !== false) ? 'class="dropdown-list open" ' : 'class="dropdown-list"' ?>>
        ...
        <li <?= ($page == 'addlots') ? 'class="active"' : '' ?>><a href="<?= Url::to(['user/import-lots']) ?>">Импортировать лоты</a></li>
        <li <?= ($page == 'editlot') ? 'class="active"' : '' ?>><a href="<?= Url::to(['user/add-lot']) ?>">Добавить лот</a></li>
      </ul>

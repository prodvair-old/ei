...
<figure class="tour-<?= $type ?>-item-01" itemscope itemtype="http://schema.org/Product">
    ...
    <figcaption class="content">
        ...
        <ul class="item-meta lot-block__info">
            ...
            <li>
                <!--                                <div -->
                <? //=(Yii::$app->user->isGuest)? 'href="#loginFormTabInModal-login" class="wish-star" data-toggle="modal" data-target="#loginFormTabInModal" data-backdrop="static" data-keyboard="false"' : 'href="#" class="wish-js wish-star" data-id="'.$lot->id.'" data-type="'.$lot->torg->type.'"'?><!-->
                -->
                <!--                                    <img src="img/star-->
        ...
        <? if ($lot->torg->type == 'zalog') { ?>
            ...
            <ul class="item-meta lot-block__info">
                <li>
                    Организация: <span
                            class="<?= ($lot->archive) ? 'text-muted' : '' ?>"> <?= isset($lot->torg->owner) ? $lot->torg->owner->title : '' ?></span>
                </li>
            </ul>
        <? } ?>
        <hr>
        <ul class="item-meta lot-block__info">
            <!--            <li>-->
            <!--                Категория: <span class="--><? //= ($lot->archive) ? 'text-muted' : '' ?><!--"-->
            <!--                                 itemprop="category"> --><? //= $lot->category->name ?><!--</span>-->
            <!--            </li>-->
            Категория:
            <?php foreach ($lot->category as $item) : ?>
                <li>
              <span class="<?= ($lot->archive) ? 'text-muted' : '' ?>"
                    itemprop="category"> <?= $item->name ?></span>
                </li>
            <?php endforeach; ?>
        </ul>
        <hr>
        <p class="mt-3"><span class="h6 line-1 <?= $priceClass ?> font16"
            <!--            --><? //=($color)? 'style="color: '.$color.'!important"': ''?><!--<temprop="price">-->
<!--            --><?//= Yii::$app->formatter->asCurrency($lot->priceHistory->price) ?><!--</span> <span class="text-muted mr-5"><s>-->
<!--            --><? //= ($lot->oldPrice)? Yii::$app->formatter->asCurrency($lot->oldPrice) : '' ?></s></span></p>
    </figcaption>
    <?= ($type == 'long') ? '</div>' : '' ?>

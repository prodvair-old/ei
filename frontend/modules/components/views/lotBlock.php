...
<!--            --><?// } ?>
            ...
            <ul class="item-meta lot-block__info">
                ...
                <?php foreach ($lot->category as $item) : ?>
                    <li>
                        <span itemprop="category">   <?= $item->category->name ?></span>
                    </li>
                <?php endforeach; ?>
            </ul>
            <hr>
            <p class="mt-3"><span class="h6 line-1 <?=$priceClass?> font16" <?=($color)? 'style="color: '.$color.'!important"': ''?> itemprop="price"><?= Yii::$app->formatter->asCurrency($lot->start_price)?></span> <span class="text-muted mr-5"></span></p>
        </figcaption>
        <?=($type == 'long')? '</div>' : ''?>
